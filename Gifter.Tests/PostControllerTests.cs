...
namespace Gifter.Tests
{
    public class PostControllerTests
    {
        ...
        [Fact]
        public void Get_Returns_Post_With_Given_Search_Parameter()
        {
            var postCount = 20;
            var posts = CreateTestPosts(postCount);
            ...
            // Act
            var result = controller.Search(testPostParameter, true);

            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result);
            var actualPosts = Assert.IsType<List<Post>>(okResult.Value);

            foreach(Post post in actualPosts)
            {
                
                Assert.True(post.Caption.Contains(testPostParameter) || post.Title.Contains(testPostParameter));
            }
